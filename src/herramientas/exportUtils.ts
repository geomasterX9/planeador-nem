...
// Limpiador de texto para evitar caracteres corruptos
...
const createCell = (text: string, isHeader: boolean = false, widthPct: number = 0, alignment: AlignmentType = AlignmentType.LEFT, bgColor?: string, colSpan: number = 1) => {
  ...
  const safeString = sanitizeText(text);
  ...
  return new TableCell({
    ...
    children: [
      new Paragraph({
        alignment: alignment,
        children: [new TextRun({ text: safeString, bold: isHeader, size: 18, color: textColor, font: "Calibri" })],
      }),
    ],
  });
};

// --- EL FILTRO PURIFICADOR DE IMÁGENES ---
const base64ToArrayBuffer = (base64DataUrl: string) => {
  try {
    if (!base64DataUrl || !base64DataUrl.includes(',')) return null;
    
    // .replace(/\s/g, '') es la magia que elimina saltos de línea y espacios corruptos
    const base64String = base64DataUrl.split(',')[1].replace(/\s/g, ''); 
    ...
    console.error("Imagen bloqueada para evitar corrupción:", error);
    return null;
  }
};
...
export const exportToWord = async (projectData: any, plannedItems: any[], actividades: Record<string, string>, evaluationData?: any) => {
  ...
  // Procesamos los logos pasándolos por el filtro purificador
  const bufferIzq = base64ToArrayBuffer(projectData.logoIzquierdo);
  const bufferDer = base64ToArrayBuffer(projectData.logoDerecho);

  // CONSTRUCCIÓN DEL ENCABEZADO CON LOGOS
  const headerCells = [];
  ...
  if (bufferIzq) {
    headerCells.push(new TableCell({
      ...
      verticalAlign: VerticalAlign.CENTER,
      children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new ImageRun({ data: bufferIzq, transformation: { width: 90, height: 90 } })] })]
    }));
  } else {
  ...
  headerCells.push(new TableCell({
    ...
    children: [
      ...
      new Paragraph({ children: [new TextRun({ text: sanitizeText(projectData.escuela || "NOMBRE DE LA ESCUELA"), bold: true, size: 18, font: "Calibri" })], alignment: AlignmentType.CENTER }),
      new Paragraph({ children: [new TextRun({ text: `CLAVE: ${sanitizeText(projectData.cct)}    TURNO: ${sanitizeText(projectData.turno)}`, bold: true, size: 18, font: "Calibri" })], alignment: AlignmentType.CENTER }),
  ...
  if (bufferDer) {
    headerCells.push(new TableCell({
      ...
      borders: { top: { style: BorderStyle.NONE, size: 0 }, bottom: { style: BorderStyle.NONE, size: 0 }, left: { style: BorderStyle.NONE, size: 0 }, right: { style: BorderStyle.NONE, size: 0 } },
      verticalAlign: VerticalAlign.CENTER,
      children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new ImageRun({ data: bufferDer, transformation: { width: 90, height: 90 } })] })]
    }));
  } else {
    headerCells.push(new TableCell({ width: { size: 15, type: WidthType.PERCENTAGE }, borders: { top: { style: BorderStyle.NONE, size: 0 }, bottom: { style: BorderStyle.NONE, size: 0 }, left: { style: BorderStyle.NONE, size: 0 }, right: { style: BorderStyle.NONE, size: 0 } }, children: [new Paragraph({ children: [new TextRun(" ")] })] }));
  }
      ...
      new TableRow({
        children: [
          ...
          new TableCell({
            ...
            children: actText.split('\n').map(line => new Paragraph({ children: [new TextRun({ text: sanitizeText(line), size: 18, font: "Calibri" })], spacing: { after: 100 } }))
          })
        ]
      })
    );
        ...
        children: [
          new TableCell({
            ...
            children: [
              ...
              new Paragraph({ children: [new TextRun({ text: sanitizeText(projectData.maestro || "Nombre del Docente"), size: 18, font: "Calibri" })], alignment: AlignmentType.CENTER, spacing: { before: 50 } })
            ]
          }),
          new TableCell({
            width: { size: 50, type: WidthType.PERCENTAGE },
  ...
  saveAs(blob, `Planeacion_${sanitizeText(projectData.proyecto).replace(/[^a-zA-Z0-9 ]/g, "") || "NEM"}.docx`);
};