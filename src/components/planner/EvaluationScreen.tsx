  ...
  const generateAIEvaluation = async () => {
    ...
    try {
      ...
      if (['Listas de cotejo', 'Guías de observación', 'Escalas estimativas'].includes(activeTab)) {
        ...
        if (activeTab === 'Listas de cotejo') setCriteriosCotejo(items.slice(0, 5));
        if (activeTab === 'Guías de observación') setCriteriosObservacion(items.slice(0, 5));
        if (activeTab === 'Escalas estimativas') setCriteriosEscala(items.slice(0, 5));
      } else if (activeTab === 'Cuestionarios') {
        setTextoCuestionario(aiText.replace(/\*\*/g, ''));
      } else if (activeTab === 'Exámenes escritos') {
        setTextoExamen(aiText.replace(/\*\*/g, ''));
      } else if (activeTab === 'Rúbricas') {
        const rows = aiText.split('\n').filter(r => r.trim() !== '');
        const newRubrica = rows.map((row: string, idx: number) => {
          const parts = row.split('|||').map(p => p.trim());
          return {
            id: Date.now() + idx,
            criterio: parts[0] || "Criterio generado",
            nivel4: parts[1] || "",
            nivel3: parts[2] || "",
            nivel2: parts[3] || "",
            nivel1: parts[4] || ""
        ...
        if (newRubrica.length > 0) setCriteriosRubrica(newRubrica);
      }
